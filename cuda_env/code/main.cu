...
int main()
{
...

    {
        ...
        int output_size = 10;
        int batch_size = 100;
        SoftMaxWithLoss Layer3(input_size, output_size, batch_size, true);
        ...
        for(int row = 0;row<h_teacher.height;++row)
        {
            for(int col = 0;col<h_teacher.width;++col)
            {
                printf("%d,%d,%f,%f\n", row, col, 100*ans_x.elements[row * h_x.width + col], h_y_soft.elements[row * h_x.width + col] - h_teacher.elements[row * h_x.width + col]);
            }
        }
